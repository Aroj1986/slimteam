...
const MyCalendar = () => {
  ...
  const handleSelect = async ({ start, end }) => {
    ...
    if (title) {
      axios.post('http://localhost:8888/book-online', {start, end, title}).then(({data}) => setEvents([...events, data ])).catch(err => console.log(err))
    }
  ...
  const handleEdit = ({ _id, title }) => {
    ...
    if (newTitle) {
      ...
      axios.put(`http://localhost:8888/book-online/${_id}`, { title: newTitle })
      .then(response => {
        console.log('Event updated successfully', response.data);
        const findEvent = events.findIndex(event => event._id === _id )
        if (findEvent === -1) throw new Error('Could not find event in the events state')
  ...
  const handleDelete = ({ _id }) => {
    ...
    setEditingEvent(null);
    axios.delete(`http://localhost:8888/book-online/${_id}`)

    // Inside of then - change the event state to remove the element that was deleted (TODO - frontend)
    // Backend - needs to get the ID out of the params (findByIdAndDelete method)
    .then(response => {
  ...
  const eventPropGetter = (event) => {
    const isBooked = events.some((e) => e !== event && moment(event.start).isBetween(e.start, e.end, null, '[]'));
    return {
